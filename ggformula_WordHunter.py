...
def create_pdf_content(words):
    ...
    words_per_page = 15
    words_to_process = words[:words_per_page]
    
    table_data = []
    
    # Create a single row for the bold words
    bold_row = [Paragraph(f"<b>{word}</b>", penmanship_style) for word in words]
    table_data.append(bold_row)
    
    # Create 4 more rows with the dotted/normal style
    for _ in range(4):
        clone_row = [Paragraph(word, normal_style) for word in words]
