...
def get_all_words():
    ...
    custom_words = set(custom_file.read_text(encoding="utf-8", errors="ignore").splitlines()) if custom_file.exists() else set()
    large_file = Path("data/large_words.txt")
    large_words = set(large_file.read_text(encoding="utf-8", errors="ignore").splitlines()) if large_file.exists() else set()
    merged = wordnet_words | extra_words | dolch_words | custom_words | large_words
    ...
    try:
        ...
        for line in wt.splitlines():
            ...
            if line.startswith("# ") and not line.startswith("##"):
                txt=line.lstrip("# ").strip().replace("'''","").replace("''","").replace("[[","").replace("]]","")
...
def aggregate_meanings(word: str):
    ...
    if wn["definitions"]:
        agg_defs.extend(wn["definitions"]); agg_syns |= set(wn["synonyms"]); sources.append("WordNet")
    tasks=[]
    with ThreadPoolExecutor(max_workers=2) as ex:
        ...
        for fut in as_completed(tasks):
            ...
            agg_syns |= set(info.get("synonyms", []))
...
def create_tracer_pdf_buffer(words):
    ...
    font_main="Helvetica-Bold"; font_size_main=28
    font_clone="Helvetica-Bold"; font_size_clone=28
    ...
    if st.session_state.get('search_triggered') and 'matches' in st.session_state:
        ...
        if matches:
            ...
            with ThreadPoolExecutor(max_workers=12) as ex:
                ...
                for f in as_completed(futs): data_rows.extend(f.result())
            ...
            # Build view (Sources column removed from view only)
            ...
            # Excel download (full with sources included)
            towrite=BytesIO()
