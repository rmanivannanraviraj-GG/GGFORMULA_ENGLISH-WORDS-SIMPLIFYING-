...
# --- Word lists: WordNet + Dolch + custom + large word list ---
@st.cache_data
def get_all_words():
    ...
    custom_words = set()
    if custom_file.exists(): custom_words = set(custom_file.read_text().splitlines())
    # Large 100k+ word list file
    large_file = Path("data/large_words.txt")
    large_words = set()
    if large_file.exists(): large_words = set(large_file.read_text().splitlines())
    return sorted(wordnet_words.union(dolch_words).union(custom_words).union(large_words), key=lambda x:(len(x), x.lower()))

def find_matches(words, suffix, before_letters):
    suf = suffix.lower(); matched = []
    for w in words:
        if w.lower().endswith(suf):
            if before_letters==0 or len(w)-len(suf)==before_letters: matched.append(w)
...
with st.container():
    ...
    if st.session_state.get('search_triggered') and 'matches' in st.session_state:
        ...
        if matches:
            ...
            for word in matches:
                ...
                if not syns: data_rows.append({"Word":word,"Word Type":"-","English":"-","Tamil":"-"})
                else:
                    for syn in syns:
    ...
    else: st.info("Please enter a suffix and click 'Apply' to see definitions.")
